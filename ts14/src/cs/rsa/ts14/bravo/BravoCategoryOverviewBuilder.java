...
public class BravoCategoryOverviewBuilder implements ReportBuilder {
	...
	public void buildBegin() {
		...
		// Initialize work categories to ensure that all categories are shown +
	...
	public void buildEnd() {
		...
		// The percentace calculations are apparently done without calculating
		// the cosultancy hours
		...
		for (ClassType classType : ClassType.values()) {
			// Workaround for ClassType enum items not being in the specified
			// order
			...
			resultStringBuilder.append(String.format(engLocale, "%-21s%9.1f (%3.0f%%)\n", getClassString(classType), classHours, classPercentOfTotal));

			// Iterate through all workCategoryToHoursMap items, and append the
			// work categories that belongs to this work class
